class Coc::SkillSetsController < ApplicationController
  ...
  def create
    ...
    skill_hash = params.require(:coc_skill_set).permit(SKILL_SET_ATTRIBUTES)
  ...
  def extract_skill_sets( skill_set )
    skill_set.select do |k,v|
      Coc::SkillSet::ALL_SKILLS.reject do |k,v|
        Coc::SkillSet::SKILL_CATEGORIES.keys.include? k
      end.keys.include? k.to_sym
    end
  end

  def extract_sub_skill_sets( skill_set )
    Coc::SkillSet::SKILL_CATEGORIES.reduce({}) do |m, s|
      k = s[0].to_s
      h = {}
      [0,1,2].each do |i|
